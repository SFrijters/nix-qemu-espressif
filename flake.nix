  ...
  outputs =
      ...
      mkCheck =
        ...
        let
          ...
          executablesPerVariant = {
            "qemu-espressif" = [ "qemu-system-xtensa" "qemu-system-riscv32" ];
          ...
          mkCheckVersion = exe: "echo ${lib.getExe' pkg exe}\n${lib.getExe' pkg exe} --version | grep '${pkg.version}'\n";
          mkCheckMinimal = exe: "${lib.optionalString isMinimal "!"} ${lib.getExe' pkg exe} --display help | grep -v -e 'Available\\|none\\|dbus'\n";
          mkCheckMachines = exe: lib.concatMapStrings (arch: "${lib.getExe' pkg exe} --machine help | grep '^${arch} '\n") archPerExecutable.${exe};
    ...
    in
    {
      ...
      packages = forAllSystems (pkgs: rec {
        ...
        qemu-esp32-minimal = pkgs.callPackage ./packages/qemu-espressif { enableEsp32c3 = false; minimal = true; };
        qemu-esp32c3-minimal = pkgs.callPackage ./packages/qemu-espressif { enableEsp32 = false; minimal = true; };
      });

      checks = lib.mapAttrs (system: perSystem: lib.mapAttrs (_: pkg: mkCheck system pkg) (lib.filterAttrs (n: _: n != "default") perSystem)) self.packages;
